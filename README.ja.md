...
- **矢印キーは常に「名前順」（数字を数値として比較。`2.jpg`は`10.jpg`より前）
  で移動する。** これは Finder ウィンドウを「名前」で並べ替え/整頓した時と
  同じ順番。**アイコン表示でアイコンの並びが「名前」以外（手動配置・種類順
  など）になっていると、画面上で隣に見えるファイルと矢印キーが移動する先が
  ズレる**（実機で確認）。リスト表示にするか、アイコン表示のまま「名前」で
  整頓すれば一致する。
...
をまとめた `TigerQuickLook-0.6.zip` / `.dmg` を置いている
(zip/dmg/app は gitignore 対象。MacRumors 投稿文の下書き `macrumors_thread_draft.md`
も同じフォルダ)。GitHub Releases (`v0.6`) にも同じ zip/dmg を添付済み。
